<section class="home-section reveal-section" id="internships" aria-labelledby="internships-title">
  <header class="section-heading">
    <span class="section-heading__index">05</span>
    <div>
      <h2 id="internships-title">Internships</h2>
    </div>
  </header>

  <div class="internship-list">
    <article class="internship-card">
      <div class="internship-card__logo">
        <img class="internship-card__logo-light"
             src="{{ '/assets/images/simplexity_logo_en_dark.svg' | relative_url }}"
             alt="Simplexity logo">
        <img class="internship-card__logo-dark"
             src="{{ '/assets/images/simplexity_logo_en.svg' | relative_url }}"
             alt="Simplexity logo">
      </div>

      <div class="internship-card__content">
        <h3 class="internship-card__company">
          <a href="https://www.simplexityrobotics.com/#home" target="_blank" rel="noopener noreferrer">Simplexity Robotics</a>
        </h3>
        <p class="internship-card__position">Research and Development Intern</p>
        <p class="internship-card__meta">
          <time datetime="2026-06">Jun 2026</time> – <time datetime="2026-08">Aug 2026</time> · Shanghai, China
        </p>

        <ul class="internship-card__work" aria-label="Responsibilities">
          <li>Cloud-based Models</li>
          <li>VLMs</li>
          <li>Algorithm Optimization</li>
        </ul>
      </div>
    </article>
  </div>
</section>
